...
QuadTreeNodePolicy.prototype.genLevels = function(ratio) {
	...
	var max = Math.min(bbrx, bbry);

	// the compression ratio, affects the outer rings, not the inner ones
	for (var i = 0 ; i < this.maxDepth ; i++) {
		var f = i / this.maxDepth;
		var s = Math.pow(f, 10);
		var r = max * s;
		lodlevels.push(s);
	}
...
QuadTreeNodePolicy.prototype._hookupLODDebug = function() {
	...
	o.renderer.addPostRender(function(gl, mvp, mv, proj) {
		...
		var shp = o.tree.LODSpheres(), max = shp.length;
...
QuadTreeNodePolicy.prototype.start = function() {
    ...
    var makeId = function(n) {
        ...
        var depth = 19 - node.depth;

        id[l.point.constructor.key] = l.point.queryFor(bbox, depth - 1, depth);
	...
	var depthToTraverse = function(ratio) {
		return 6;
		
		// given the ratio of the view and how far we're from it, we figure how deep we want to go into our tree
		// it makes no sense to load a full tree when all we need is a little bit of it
		//
		// Note that our quad-tree leave nodes have depth 0
		if (ratio > 0.3) {
			return 7;
		}
		else if (ratio < 0.005) {
		...
		// good enough curve, for domain 0 -> 1, goes 0 -> 2 ... with steep curve up towards to after like 0.75
		// remember to noramalize ratio to 0 -> 1
		var nratio = 1 - (ratio / 0.3);
		var f = Math.exp(1.1 * Math.pow(nratio, 2)) - 1;
		var depth = 7 - 0.5 * f * 7;

		console.log("ratio params:", nratio, f, depth);
    ...
    var trigger = new TriggeredDispatch(500, function(view) {
	    ...
	    var bailAtDepth = depthToTraverse(ratio);

	    console.log("view-params:", viewDist, ratio, bailAtDepth);
	    ...
	    var ll = o.genLevels(ratio);
	    o.tree.updateLODSpheres(ll);
	    
        o.tree.lodQuery(pos, bailAtDepth, function(b) {
            nodes.push(b);
        });
