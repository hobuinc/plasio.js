    ...
    var checkOwner = function(nowait) {
        ...
        util.get(url, function(err, res) {
            ...
            if (r.node && r.node.value && r.node.value !== o.lastOwner) {
                ...
                o.emit("new-owner", JSON.parse(o.lastOwner));
            }
...
Session.prototype.setOwner = function(ownerName, owner, cb) {
    util.put(this.baseUrl + "/owner", {value: JSON.stringify({name: ownerName, id: owner})}, cb);
};
...
P2PNode.prototype.startBroadcast = function(startupProps) {
    ...
    this.makePeer(this.nodeName, function(err, peer) {
        ...
        o.session.setOwner(o.nodeName, peer.id, function() {
            o.bstate = {
                conns: {}
            };
...
P2PNode.prototype.startAsClient = function(ownerName) {
    ...
    this.makePeer(this.nodeName, function(err, peer) {
        ...
        var setupClient = function(ownerInfo) {
            var owner = ownerInfo.id;
            var ownerName = ownerInfo.name;

            console.log("======", owner, ownerName, o.peer.id);

            if (owner === o.peer.id)
                return; // we are the owner, so don't do any client side stuff

            if (o.onviewer)
                o.onviewer(e, owner, ownerName);
            
            var conn = o.peer.connect(owner, {metadata: {name: o.nodeName}}); 

            conn.on('open', function() {
                o.cstate = { owner: owner, conn: conn };
                e.emit('open');
            });
        ...
        if (o.session.lastOwner) {
            setupClient(JSON.parse(o.session.lastOwner));
        }

        o.session.on("new-owner", function(info) {
            if (info.owner !== o.peer.id)
                setupClient(info);
        });
