...
export class GeoTransform {
    ...
    transform(arg, sourceTransform, destinationTransform) {
        ...
        // render
            ...
            // for bounds, the x is switched, the render coordinate system goes from left to right, so we need to reverse
            // it here
            normalized = (p.length === 3) ? [-p[0], p[2], p[1]] : [-p[3], p[2], p[1], -p[0], p[5], p[4]];
        }
        ...
        if (destinationTransform === 'tree') destination = normalized.slice(0);
            ...
            let p = normalized;
            // for bounds, the x is switched, so that render coordinate system always goes left to right
            console.log(p.toString(), scale.toString());
            p = (p.length === 3) ? [-p[0], p[2], p[1]] : [-p[3], p[2], p[1], -p[0], p[5], p[4]];
            const s = [scale[0], scale[2], scale[1]];
            destination = p.map((v, i) => v * s[i % 3]);

            console.log('p', sourceTransform, arg, p, destination, s);
        }
