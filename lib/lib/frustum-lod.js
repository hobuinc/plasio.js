...
class TreeInfoCache {
    ...
    async nodeInfo(node) {
        ...
        if (pointCount == null) {
            ...
            if (this._isPrefetchLevel(node) && node.depthEnd < stopSplitDepth) {
...
class Box {
     ...
     *
     * @param {Number} stopSplitDepth The depth at which to stop subdividing into 8 children
     * @param {Number} hardStopDepth The absolute maximum depth to go to.
...
class TreeWalker {
    ...
    _acceptNode(node, fAdd) {
        if (!this.nodesInScene[node.id]) {
            // this node was in scene, so remove it
            fAdd(node);
            this.nodesInScene[node.id] = node;
        }
    }
    ...
    async _doWalk(node) {
        ...
        const [vf, radiusOnScreen] = FrustumLODNodePolicy._screenSizeForBox(node, fov,
        ...
        // No points, so no point showing
        const pointCount = await this.treeInfoCache.nodeInfo(node);
        if (pointCount == 0)
            return this._rejectNode(node, fRemove);

        // this node seems to pass acceptance
        // TODO: WEIGHTS! Wait for renderer to add prioritized loading
        this._acceptNode(node, fAdd);

        const children = node.subdivide(stopSplitDepth, hardStopDepth);
        if (children.length == 1) {
            // end of tree, accept this node
            this._acceptNode(children[0], fAdd);
        }
     ...
     * @param {Number} stopSplitDepth The stop split depth.
     * @param {Number} hardStopDepth The hard stop depth.
     ...
     */
    async walk(node, frustum, eye, target, fov, clientHeight, rejectAtScreenSize, stopSplitDepth,
               hardStopDepth, fAdd, fRemove) {
        this.activeParams = {
            frustum: frustum,
            eye: eye,
            target: target,
...
export class FrustumLODNodePolicy extends EventEmitter {
    ...
    async _updateView(view, rootNodeData) {
        ...
        await this.treeWalker.walk(
            ...
            (node) => this.renderer.addPointBuffer(this._makeId(node)),
            (node) => this.renderer.removePointBuffer(this._makeId(node))
        );
