...
Box.prototype.subdivide = function(zeroReturn) {
    ...
    if (depth < STOP_SPLIT_DEPTH) {
        ...
        boxes[0].splitRange = [depth, HARD_STOP_DEPTH];
    }
...
class Preloader {
    ...
    walkTree(baseNode, cb, done) {
        ...
        let q = async.queue((node, done1) => {
            ...
            let splitRange = node.splitRange;
            let depthEnd, depthBegin;
            if (splitRange) {
                let [s, e] = splitRange;
                depthEnd = o.startDepth + e;
                depthBegin = o.startDepth + s;
            }
            else {
                depthEnd = o.startDepth + node.depth;
                depthBegin = (depthEnd === o.startDepth) ? 0 : (depthEnd - 1);
            }

            let params = o.loader.queryFor(bbox, depthBegin, depthEnd);
            o.loader.constructor.load(params, (err, data) => {
                console.log("load complete", params, data);
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var makeId = function(node) {
        ...
        let splitRange = node.splitRange;
        var depthEnd, depthBegin;

        if (splitRange) {
            let [s, e] = splitRange;
            depthBegin = startDepth + s;
            depthEnd = startDepth + e;
        }
        else {
            depthEnd = startDepth + node.depth;
