...
Box.STOP_SPLIT_DEPTH = 16;
Box.HARD_STOP_DEPTH = 32;
...
Box.prototype.subdivide = function() {
    ...
    var boxes = [];
    if (depth < Box.STOP_SPLIT_DEPTH) {
        // regular split, no need to do anything special, just split 8 ways
        boxes = _split(genid, this);
    }
    else if (depth === Box.STOP_SPLIT_DEPTH) {
        // bulk load to load all levels to full depth
        boxes = [
            new Box(x, y, z, w, h, d, id(), this.depthBegin + 1, Box.HARD_STOP_DEPTH, this)
        ];
    }
...
class Preloader {
    ...
    walkTree(baseNode, cb, done) {
        ...
        let q = async.queue((node, done1) => {
            ...
            let depthEnd = node.depthEnd;
            let depthBegin = node.depthBegin;

            let params = o.loader.queryFor(bbox, depthBegin, depthEnd);
            console.log("qierying...");
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var makeId = function(node) {
	    ...
	    if (l.point) {
            id[l.point.constructor.key] =
                l.point.queryFor(bbox, node.depthBegin, node.depthEnd);
        }

	    if (l.overlay) {
            id[l.overlay.constructor.key] =
                l.overlay.queryFor(bbox);
        }
    ...
    var trigger = new TriggeredDispatch(500, function(view) {
        ...
        let loader = new Preloader(o.loaders.point, center);
        lastLoader = loader;
