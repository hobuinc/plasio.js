...
class TreeWalker {
    ...
    _acceptNode(node, pointCount, weight, revision, fAdd) {
        if (!this.nodesInScene[node.id]) {
            fAdd(node, pointCount, weight);
            ...
            // may be the node weight has changed, this means that it has loading priority
            //
            const savedNodeInfo = this.nodesInScene[node.id];

            if (savedNodeInfo.weight != weight) {
                savedNodeInfo.weight = weight;
                // re-add with new weight
                fAdd(node, pointCount, weight);
            }

            if (savedNodeInfo.revision < revision)
                savedNodeInfo.revision = revision;
        }
    }
    ...
    _screenSizeForBox(eye, box, cameraFOV, clientHeight) {
        const eyeToBoxDistance = vec3.distance(eye,
            vec3.add(this._nodePosition, box.position, this.renderSpaceTreeOffset));

        if (box.radius > eyeToBoxDistance) {
            return Number.MAX_VALUE;
        }
        else {
            let fov = cameraFOV * Math.PI / 180.0;
            let slope = Math.tan(fov / 2);
            let pf = (0.5 * clientHeight) / (slope * eyeToBoxDistance);

            return box.radius * pf;
        }
    }
    ...
    async _doWalk(node, revision) {
        ...
        this._acceptNode(node, pointCount, weight, revision, fAdd);

        const children = node.subdivide();
        if (children.length == 1) {
            // end of tree, accept this node
            const pointCount = await this.treeInfoCache.nodeInfo(children[0]);
            if (pointCount > 0)
                this._acceptNode(children[0], pointCount, weight, revision, fAdd);
        }
...
/**
 * Manages which buffers are loaded vs which aren't based on our point count limit
 */
class LoadQueue {
    /**
     * Constructs a load queue which keeps track of point budget and appropriately loads buffers and tries to keep point under the requested limit.
     * Note that, the total number of loaded points may still exceed the set limit. The load queue tries to load as many buffers as possible
     * to reach the specified limit, loading the last buffer to reach this limit may exceed the set limit.  No additional buffers are loaded beyond
     * that point.
     * @param renderer [Object] The renderer to add the buffer to.
     * @param pointCountLimit [Number] The maximum number of points to be allowed to pass through to the renderer, defaults to 2,000,000.
     */
    constructor(renderer, pointCountLimit) {
        this.renderer = renderer;
        this.limit = (pointCountLimit || 4000000);
        this.bufferList = [];
    }


    /**
     * Add the specified buffer ID with the given weight.
     * @param id [String] An arbitrary string that uniquely identifies this buffer.
     * @param rendererBufferId [String] The buffer ID as passed to the renderer.
     * @param pointCount [Number] The number of points in this buffer.
     * @param weight [Number] The weight of the buffer.
     */
    add(id, rendererBufferId, pointCount, weight) {
        // we may already know of this buffer, since it may have been added before but with a different priority, in which
        // case it needs to be updated and re-inserted in the correct spot.
        let bufferSpec = null;
        const currentIndex = this.bufferList.findIndex(b => b.id === id);

        if (currentIndex === -1) {
            // this is a new buffer
            bufferSpec = {
                id: id,
                rendererBufferId: rendererBufferId,
                count: pointCount,
                weight: weight,
                loaded: false
            };
            ...
            bufferSpec = this.bufferList[currentIndex];
            this.bufferList.splice(currentIndex, 1);
        }

        // find the spot for it
        let position = -1;
        for (let i = 0, il = this.bufferList.length ; i < il && position === -1 ; i ++) {
            const b = this.bufferList[i];

            if (b.weight < bufferSpec.weight) {
                position = i;
            }
        }

        if (position === -1) {
            this.bufferList.push(bufferSpec);
        }
        else {
            this.bufferList.splice(position, 0, bufferSpec);
        }

        // Now make sure we are under budget
        this._budgetPoints();
    }

    /**
     * Remove the buffer with given ID.
     * @param id [String] The ID of the buffer to remove.
     */
    remove(id) {
        const index = this.bufferList.findIndex(b => b.id === id);
        if (index >= 0) {
            const b = this.bufferList[index];

            if (b.loaded)
                this.renderer.removePointBuffer(b.rendererBufferId);
            this.bufferList.splice(index, 1);

            // re-adjust loaded buffers to make up for point budget
            this._budgetPoints();
        }
    }
    ...
    _budgetPoints() {
        ...
        // first check if there are any buffers in our budget list which fall under the limit
        let i = 0, il = this.bufferList.length;
        for (i = 0, il = this.bufferList.length ; i < il && count < this.limit ; i ++) {
            const b = this.bufferList[i];
            count += b.count;

            if (!b.loaded) {
                b.loaded = true;
                this.renderer.addPointBuffer(b.rendererBufferId, {weight: b.weight});
            }
        }

        // now check if there are any buffers beyond the budget list which are loaded
        for ( ; i < il ; i++) {
            const b = this.bufferList[i];
        ...
        console.log("load-queue: point budget at:", count);

        // console.log("load-queue", this.bufferList.map(n => (n.id + ":" + n.weight)).join(", "));
...
export class FrustumLODNodePolicy extends EventEmitter {
     ...
     * @param {Number} [params.pointBudget] The point budget hint.
    ...
    constructor(namespace, loaders, renderer, params) {
        ...
        this.pointBudget = util.checkParam(params, 'pointBudget');
        ...
        // The load queue to manage point buffer budget
        this.loadQueue = new LoadQueue(this.renderer, this.pointBudget);
    ...
    async start() {
        ...
        this.clearAll = () => {
            this.treeWalker.clearAllNodes(n => {
                this.loadQueue.remove(n.treePath);
            });
        };
    ...
    async _updateView(view, rootNodeData) {
        ...
        this.treeWalker.walk(
            ...
            (node, pointCount, weight) => this.loadQueue.add(node.id, this._makeId(node), pointCount, weight),
            (node) => {
                this.loadQueue.remove(node.id);
