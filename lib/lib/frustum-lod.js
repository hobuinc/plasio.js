        ...
        if (pointCount == null) {
            ...
            if (this._isPrefetchLevel(node) && node.depthEnd < node.stopSplitDepth) {
                ...
                for (let i = 0 ; i < children.length ; i ++) {
                    const n = children[i];
                    const res = await this.loader.loadHierarchyInfo(
                        n.bounds(), this.geoTransform, 9, 10
                    );
                    console.log(n, res);
                }
            ...
            const treespace = this.geoTransform.transform(node.bounds(), 'render', 'tree');
            console.log(treespace.toString() + ":" + pointCount);
            return pointCount;
        }
    }
}
...
FrustumLODNodePolicy.REJECT_ON_SCREEN_SIZE_RADIUS = 800;
