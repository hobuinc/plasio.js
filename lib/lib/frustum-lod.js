...
Box.prototype.subdivide = function(zeroReturn) {
    ...
    var boxes = [
        // bottom quad
        new Box(x, y, z, w/2, h/2, d/2, id0, depth, this), new Box(x + w/2, y, z, w/2, h/2, d/2, id1, depth, this),
        new Box(x, y + h/2, z, w/2, h/2, d/2, id2, depth, this), new Box(x + w/2, y + h/2, z, w/2, h/2, d/2, id3, depth, this),

        // top quad
        new Box(x, y, z + d/2, w/2, h/2, d/2, id4, depth, this), new Box(x + w/2, y, z + d/2, w/2, h/2, d/2, id5, depth, this),
        new Box(x, y + h/2, z + d/2, w/2, h/2, d/2, id6, depth, this), new Box(x + w/2, y + h/2, z + d/2, w/2, h/2, d/2, id7, depth, this)
    ];
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var center = bb.center();
	var startDepth = this.baseDepth || 8;
    ...
    var makeId = function(node) {
        ...
        var depthEnd = startDepth + node.depth;
    ...
    var traverseParents = function(nodes) {
        // get all of the parents for each of the nodes
        var allParents = nodes.map(function(n) {
            var nnn = [];
            var tn = n;
            while(tn) {
                nnn.push(tn);
        ...
        allParents = Array.prototype.concat.apply([], allParents);
        allParents = _.uniq(allParents, 'id');
    ...
    var trigger = new TriggeredDispatch(500, function(view) {
        ...
        while(nodes.length > 0) {
            ...
            for (var i = 0, il = children.length ; i < il ; i ++) {
                ...
                let childDistanceFromY = Math.abs(child.position[1] - midY);
                ...
                let weight = child.radius / (cameraToChildDistance * targetToChildDistance * childDistanceFromY);
        ...
        // visible may not be necessarily sorted at this point
        //
        visible.sort(function(a, b) {
            return b.weight - a.weight;
        });

        var NODES_TO_KEEP = 100;

        var before = visible.length;

        if (visible.length > NODES_TO_KEEP) {
            visible = visible.slice(0, NODES_TO_KEEP);
        }

        var after = visible.length;

        var allParents = traverseParents(visible);

        // add our parents to the visible set
        visible = visible.concat(allParents);
        visible = _.uniq(visible, 'id');


        var after_parents = visible.length;

        console.log("visible clip:", after, "/", before, "/", after_parents);
        ...
        var time_taken = util.perf_end(tt);

        console.log("t:", time_taken.toFixed(5) + "ms");
        console.log("e:", strNodes(o.nodes));
        console.log("v:", strNodes(visible));
        console.log("n:", strNodes(newNodes));
        console.log("c:", strNodes(commonNodes));
        console.log("a:", strNodes(nodesToRemove));
        console.log("p:", strNodes(allParents));
