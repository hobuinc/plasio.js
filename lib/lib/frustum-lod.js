...
class TreeInfoCache {
    ...
    __loadInfo(node, data) {
        ...
        if (children.length === 1) {
            // the count is the sum of all nodes counts
            let sum = _.reduce(data, (acc, n) => {
                return acc + (n.count == null ? 0 : n.count);
            }, 0);
            ...
            _.forEach(data, (v, k) => {
                ...
                if (k !== 'count') {
                    ...
                    this.__store(key, v.count);

                    // recur down if it has more children
                    if (_.size(v) > 1) { // the 1 key we have is 'count'
    ...
    nodeInfo(node, cb) {
        ...
        if (info == null) {
            ...
            if (this.__isPrefetchLevel(node)) {
                ...
                console.log("TREE: PREFETCH!");
                this.loader.loadHierarchyInfo(bbox, this.rangeX, node.depthEnd, node.depthEnd + stepSize, (err, res) => {
                    // flatten it out and add all keys into the cache
                    let r = res || {count: 0};

                    this.__store(k, r.count);
...
var FrustumLODNodePolicy = function(loaders, renderer, params) {
...
FrustumLODNodePolicy.TREE_PREFETCH_STEP = 3;
...
class Preloader {
    ...
    walkTree(baseNode, cb, done) {
        ...
        let q = async.queue((node, done1) => {
            o.cache.nodeInfo(node, (count) => {
                if (node.id.indexOf("R0") === 0 || node.id.indexOf("R2") === 0)
                    console.log("TREE:", node.id, count, this.cache.__makeURL(node), this.cache.__makeViewURL(node))

                // if this response gives us an error or zero points no point going down this tree
                if (count > 0 && !o.aborted) {
                    cb(node);
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var makeId = function(node) {
        ...
        l.forEach(e => {
            id[e.constructor.key] = e.queryFor(queryParams);
        });
