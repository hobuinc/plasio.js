...
function intersectFrustum(planes, box, viewMatrix) {
	var mins = [box.x, -1, box.y, 1], maxs = [box.x+box.w, 1, box.y+box.h, 1];
...
function allBoxesForThisView (box, clipPlanes, proj, view) {
	...
	var depth = Math.min(clipPlanes.length, 12);
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var makeId = function(node) {
        ...
        id[l.point.constructor.key] = l.point.queryFor(bbox, depthBegin, depthEnd);
    ...
    var trigger = new TriggeredDispatch(500, function(view) {
	    ...
	    startDepth = 20 - thisSeries.length;
	    ...
	    console.log("frustum view:", e, target);
