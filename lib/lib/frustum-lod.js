...
class TreeInfoCache {
    ...
    async nodeInfo(node) {
        ...
        if (this.loader.isEPT()) {
            // is the hierarchy loaded at all?
            const hierarchyStep = this.loader.getEPTHierarchyStep();

            // if no hierarchy is available, that means its one monolith hierarchy
            if (hierarchyStep == null) {
                if (this.nodesCache == null ||
                    Object.keys(this.nodesCache).length === 0) {
                    this.nodesCache = await this.loader.loadEPTHierarchyInfo();
                }
            }
            else {
                // hierarchy step is available, so we need to figure if this is the node we need to merge in
                ...
                const shouldFetch = (eptDepth === 0 && !this.loadedNodes.has(eptKey))
                    || (((this.nodesCache[eptKey] || 0) > 0) && (eptDepth % hierarchyStep) === 0 && !this.loadedNodes.has(eptKey));
                if (shouldFetch) {
