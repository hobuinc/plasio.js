...
Box.prototype.subdivide = function(zeroReturn) {
    ...
    else if (depth <= HARD_STOP_DEPTH) {
        boxes = [
            new Box(x, y, z, w, h, d, id0, depth, this)
        ];
    }
...
class Preloader {
    ...
    walkTree(baseNode, cb, done) {
        let o = this;
        o.nodesToLoad = [baseNode];

        var next = function() {
            let toLoad = o.nodesToLoad;
            o.nodesToLoad = []; // nodes for next pass

            let completed = 0,
                pending = toLoad.length;
            let center = o.center;
            ...
            toLoad.forEach(node => {
                ...
                let params = o.loader.queryFor(bbox, depthBegin, depthEnd);
                o.loader.constructor.load(params, function(err, data) {
                    if (o.aborted)
                        return; // if aborted stop doing everything

                    completed++;
                    if (completed === pending) {
                        // this was the last callback we were waiting for
                        raf(next);
                    }

                    // if this response gives us an error or zero points no point going down this tree
                    if (err || data.totalPoints === 0)
                        return;

                    // this node had some data in it,
                    cb(node);

                    // if
                    if (node.wc && node.wc.length > 0)
                        o.nodesToLoad = o.nodesToLoad.concat(node.wc);
                });
            });
        };
...
FrustumLODNodePolicy.prototype.start = function() {
    ...
    var trigger = new TriggeredDispatch(500, function(view) {
        ...
        // turn the linear list into a heirarchy
