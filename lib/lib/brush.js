...
/**
 * When a brush props transfer function is called, the direction determines the flow of parameters
 *
 * @property {number} MAIN_TO_WORKER The parameters will flow from main UI thread to a worker.
 * @property {number} WORKER_TO_MAIN The parameters will flow from workers to main UI thread.
 */
 ...
 * @property {Object} params The parameters which are passed between agents (worker or UI)
 * @property {Array.<ArrayBuffer>} transferList An array of array buffers to transfer
 */
...
export class BaseBrush {
    ...
    /**
     * Takes the current prepped state and returns a plain JS object which will be transferred over to a webworker
     * so that the brush may be de-serialized on the other end and can be used.
     *
     * @param {TransferDirection} direction The direction the of transfer
     * @return {TransferParams} The parameters to transfer
     */
    ...
    /**
     * Ends the transfer where the transfered parameters are to be staged back into the brush for painting
     * @param {TransferDirection} direction The direction of transfer
     * @param {TransferParams} params The parameters which were returned by the beginTransfer call.
     */
     ...
     * This function should also clean any transfer serialization state which was being maintained.
