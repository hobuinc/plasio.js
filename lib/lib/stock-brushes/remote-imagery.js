...
export class RemoteImagery extends BaseBrush {
    ...
    beginTransfer(direction) {
        return {
            ...
            transferList: [this.imageData.buffer]
        }
    }
    ...
    async prepare(params, parentNode, childrenNodes) {
        ...
        this.imageData = this.image.getContext("2d").getImageData(0, 0, w, h).data;
        this.bounds = renderSpaceBounds;
        this.w = w;
        this.h = h;
    }
    ...
    async unprepare(params) {
        this.bounds = this.imageData = this.image = this.w = this.h = null;
        delete this['bounds'];
