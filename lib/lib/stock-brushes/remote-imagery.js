...
export class RemoteImagery extends BaseBrush {
     ...
     * @param {String} [params.quality] The tile quality, can be 0, 1, or 2.  Defaults to 1.
     */
    constructor(spec, scheme, name, params) {
        ...
        this.quality = parseInt(checkParam(params, 'quality', '1'));
    };
    ...
    async prepare(params, parentNode, childrenNodes) {
        ...
        this.rf = function(x, z, color) {
            ...
            const col = Math.round(maprange(bounds[0], bounds[3], x, w, 0)),
                  row = Math.round(maprange(bounds[2], bounds[5], z, h, 0));
    ...
    _splatChildImagery(destCanvas, renderSpaceBounds, childrenNodes) {
        ...
        for (let i = 0, il = childrenNodes.length ; i < il ; i ++) {
            ...
            const x1 = Math.round(maprange(renderSpaceBounds[0], renderSpaceBounds[3], rsb[0], 0, w));
            const y1 = Math.round(maprange(renderSpaceBounds[2], renderSpaceBounds[5], rsb[2], 0, h));
