...
export class OrbitalCamera extends BaseMode {
    constructor(elem, renderer, f, props) {
        ...
        super.registerHandler("double-click", ({screenPos}) => {
            var p = pickPoint(renderer, screenPos);
            if (p) {
        ...
        super.registerHandler("mouse-down", ({event, screenPos}) => {
            ...
            if (dragstate.ispanning) {
                ...
                dragstate.start_pos = pickPoint(renderer, screenPos);
            ...
            dragstate.x = screenPos[0];
            dragstate.y = screenPos[1];
            dragstate.wasMoved = false;
        });

        super.registerHandler("dragging", ({event, screenPos}) => {
            if (!dragstate.active) {
                throw new Error("Got dragging event when I am not dragging?");
            }

            dragstate.wasMoved = true;
            var dx = screenPos[0] - dragstate.x;
            var dy = screenPos[1] - dragstate.y;
        ...
        super.registerHandler("mouse-up", ({event, screenPos}) => {
        ...
        super.registerHandler("mouse-wheel", ({event, screenPos}) => {
