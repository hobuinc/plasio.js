...
OrbitalCamera.prototype.enableControls = function(enable) {
	if (enable) {
		this._bindHandlers();
	}
	else {
...
OrbitalCamera.prototype.applyState = function(state) {
	state = state || {};
...
OrbitalCamera.prototype._emitCameraProps = function(overrides, isFinal, isApplyingState) {
...
OrbitalCamera.prototype.transitionTo = function(x, y, z) {
    let newPos = [
        x || this.target[0],
        y || this.target[1],
        z || this.target[2]
...
OrbitalCamera.prototype._bindHandlers = function() {
    ...
    var hovermousemove = function(e) {
        // This handler basically just checks for hovers
        var xy = util.getxy(e);
        var p = o.renderer.pickUIPoint(xy[0], xy[1]);

        if (p) {
            o.renderer.updatePoint(p[0], null, "hover");

            if (lastHighlighted && lastHighlighted !== p[0]) {
                o.renderer.updatePoint(lastHighlighted, null, "normal");
            }
    ...
    var onmousedown = function(e) {
        ...
        // Allow user to move points around if need be
        //
        if (buttonId === 1) {
            var xy = util.getxy(e);
            var p = o.renderer.pickUIPoint(xy[0], xy[1], 10);
            dragstate.moving_point = p && p[0];

            if (dragstate.moving_point) {
                var orig_p = p[2];
                var offset = vec2.subtract(vec2.create(), xy, orig_p);

                dragstate.moving_offset = offset;

                o.renderer.updatePoint(dragstate.moving_point, null, "hover");
            }
        }
        ...
        var onmousemove = function(e) {
            ...
            if (dragstate.moving_point) {
                // if we're moving point, lets update it to new location
                var xy = util.getxy(e);
                //vec2.add(xy, xy, dragstate.moving_offset);

                var p = o.renderer.pickPoint(xy[0], xy[1]);
                if (p && !sozero(p)) {
                    o.renderer.updatePoint(dragstate.moving_point, p);
                }
                return;
            }
        ...
        var onmouseup = function(e) {
            ...
            if (dragstate.moving_point) {
                o.renderer.updatePoint(dragstate.moving_point, null, "normal");
                dragstate.moving_point = null;
            }
            else {
            ...
            document.addEventListener("mousemove", hovermousemove);
        };


        lastHighlighted = null;
        document.removeEventListener("mousemove", hovermousemove);
    ...
    te.addEventListener("mousemove", hovermousemove);
    ...
    this._unbind = function() {
        ...
        te.removeEventListener("mousemove", hovermousemove);
    };
};
