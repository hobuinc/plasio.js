...
export class OrbitalCamera extends BaseMode {
    constructor(elem, renderer, geoTransform, f, props) {
        super("OrbitalCamera");
        ...
        // special handling for target since its in geo coordinates
        if (props.target) {
            this.target = this.geoTransform.transform(props.target, 'geo', 'render');
        }
        else
            this.target = [0, 0, 0]
    ...
    serialize() {
        return {
            ...
            target: this.geoTransform.transform(this.target, 'render', 'geo'),
    ...
    deserialize(state) {
        ...
        // if target is available its in geo space, convert.
        if (state.target) {
            this.target = this.geoTransform.transform(state.target, 'geo', 'render')
        }
