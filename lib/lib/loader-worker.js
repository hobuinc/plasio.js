...
/**
 * Give a field spec with field type and size, determine which DataView
 * to use for fetching this field from binary buffer.
 * @param f The field spec
 * @returns {*} The function from DataView to use to fetch this field
 */
function fieldToFetcher(f) {
    if (f.type === 'float') {
        if (f.size === 4) return DataView.prototype.getFloat32;
        else if (f.size === 8) return DataView.prototype.getFloat64;
    }
    else if (f.type === 'signed') {
        if (f.size === 1) return DataView.prototype.getInt8;
        else if (f.size === 2) return DataView.prototype.getInt16;
        else if (f.size === 4) return DataView.prototype.getInt32;
    }
    else if (f.type === 'unsigned') {
        if (f.size === 1) return DataView.prototype.getUint8;
        else if (f.size === 2) return DataView.prototype.getUint16;
        else if (f.size === 4) return DataView.prototype.getUint32;
    }

    throw new Error(`Unrecognized field: ${f.name} with size ${f.size}`);
}
...
function collectStats(buffer, pointSize, numPoints, collectFor) {
    ...
    for (var i = 0 ; i < numPoints ; i ++) {
        for (var j = 0 ; j < collectFor.length ; j ++) {
            ...
            var currValue = bin.get(key) || 0;
            currValue ++;
            bin.set(key, currValue);
...
var unpackEPTBuffer = function(buffer, totalPoints, pointSize, schema, sourceSchema, needBitFieldSizes) {
    ...
    for (let i = 0 , il = sourceSchema.length, offset = 0; i < il ; i ++) {
        ...
        const fn = fieldToFetcher(s);

        if (setOfFields.has(s.name)) {
            fields.push([s.size, fn, offset]);
        }
...
var decompressBufferEPTBinary = function(schema, sourceSchema, rawBuffer, numPoints, geoTransform) {
    ...
    sourceSchema.forEach(function(f) {
        const fieldSize = f['size'];
        pointSize += fieldSize;
        if (f.type !== 'float32' && f.type !== 'float64') needUnpack = true;
    });
...
function processTask(task) {
    ...
    if (eptRootUrl && eptKey && eptDataStorage) {
        ...
        baseUrl = joinPath(eptRootUrl, 'ept-data', eptKey + ext);
