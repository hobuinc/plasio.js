...
import { getBinary, joinPath } from "./util";
...
class BufferDownloadPipeline {
   ...
   _notifyParent(task, status, result) {
       ...
       if (result.numPoints === 0)
           console.warn('zero point task:', task);
...
var decompressBuffer = function(schema, rawBuffer, numPoints, geoTransform) {
    ...
    const treeSpaceWorlBounds = geoTransform.coordinateSpaceBounds('tree');
    if (numPoints > 0)
        swapSpace(b, [treeSpaceWorlBounds[0], treeSpaceWorlBounds[3]],
...
function processTask(task) {
    ...
    const baseUrl = joinPath(server, 'resource', resource, 'read');
    const qs = {
        schema: schema,
        bounds: bounds,
    ...
    return getBinary(baseUrl, qs, {creds: allowCreds}).then((data) => {
        let [buffer, stats] = decompressBuffer(schema, data.response, data.numPoints, geoTransform);
        return {
            buffer: buffer,
