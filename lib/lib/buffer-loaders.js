    ...
    static async _internalLoad(params, loadParams) {
        ...
        cleanupBrushes.forEach((b, i) => {
            ...
            if (selector !== ClampSelector.NONE) {
                ...
                channelClampPicks.push(["channelClampsPick" + i, new Float32Array(pickArray)]);

                channelColorRamps.push(["channelColorRamp" + i + "Start", new Float32Array(start || [0, 0, 0])]);
                channelColorRamps.push(["channelColorRamp" + i + "End", new Float32Array(end || [1, 1, 1])]);
        ...
        const result = {
            ...
            totalPoints: totalIncomingPoints,
            attributes: attribs,
            uniforms: [["availableColors", new Float32Array(GreyhoundPipelineLoader._imageryToAvailableColors(brushes))]]
                .concat(channelClampPicks)
                .concat(channelColorRamps)
                .concat([["channelRampContribution", new Float32Array(channelRampContribution)]]),
            data: buf,
