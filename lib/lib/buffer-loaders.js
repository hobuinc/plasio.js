    ...
    for (var i = 0, il = targetBufs.length ; i < il ; i ++) {
        ...
        for (let j = 0; j < totalPoints; j++) {
            ...
            if (inRange(x, z, bounds)) {
                ...
                let imgx = Math.floor(util.maprange(x1, x2, x, 0, imageData.width)),
                    imgz = Math.floor(util.maprange(z1, z2, z, 0, imageData.height));

                for (var ci = 0, cil = _.size(propData) ; ci < cil ; ci ++) {
                    let col = readPixel(propData[ci], imgx, imgz);
                    let coloffset = propData[ci].offset * 3;

                    targetBuf[offset + coloffset + 0] = col[0];
                    targetBuf[offset + coloffset + 1] = col[1];
                    targetBuf[offset + coloffset + 2] = col[2];
                }
...
class TileLoaderProxy {
    constructor(params) {
        this.loader = new TileLoader(params.url, params.layout, params.quality);
        this.params = params;
    }
    ...
    color(point) {
        ...
        let col = Math.floor(util.maprange(bbox[0], bbox[3], x, 0, w)),
            row = Math.floor(util.maprange(bbox[2], bbox[5], z, 0, h));
...
let fieldOrBlack = function(field) {
    return function(p) {
        if (p[field]) {
            let colOffset = Math.floor(p.origin * 1000) % 360;
...
let localComputeFunctions = {
    ...
    'elevation': function(p) {
        let range = this.zrange;
        let g = util.maprange(range[0], range[1], p.y, 255, 0);
        return [g, g, g];
    },
    'color': function(p) {
        return [p.red == null ? 0 : p.red * this.colorScale,
                p.green == null ? 0 : p.green * this.colorScale,
                p.blue == null ? 0 : p.blue * this.colorScale];
    }
};
...
class LocalColorLoader {
    ...
    prep(p, cb) {
        ...
        this.zrange = updateStatsRange(p.stats.z);
        cb();
    }
...
let makeImagerySource = function(source) {
    if (source.url != null) {
        // the user specified a url, so may be this is a tile service
        let url = source.url,
            layout = (source.layout == null) ? "tms" : source.layout,
            quality = (source.quality == null) ? 1 : source.quality;

        return new TileLoaderProxy({url: url, layout: layout, quality: quality});
            
    }
    else if (source.match(/^local:\/\//)) {
        // this is a local source, so create a local loader for it
        let r = source.match(/^local:\/\/(.*)/);
        let subtype = r[1];
        
        return new LocalColorLoader({subtype: subtype});
    }
    else if (source.match(/^https?:\/\//)) {
        return new TileLoaderProxy({url: source});
    }
...
GreyhoundPipelineLoader.prototype.queryFor = function(params) {
    ...
    this.imageLoaders.forEach((s, i) => {
        imageryParams['colorSource' + i] = s.queryFor(params);
    });
...
var colorizeBufferWithTasks = function(b, schema, total, size, bbox, colorizers, params, cb) {
    ...
    let pointSize = inputPointSize  + totalColorChannels * 3;

    let hasColorFromSource = schemaHasColor(schema);

    // if input size already had color then we added an extra channel
    if (hasColorFromSource)
        pointSize -= 3;

    // allocate destination array, has all need color channels
    let farray = new Float32Array(total * pointSize);

    // How many extra floats do we have trailing each point
    let extra = hasColorFromSource ? inputPointSize - 6 : inputPointSize - 3;
    let pointExtraOffset = hasColorFromSource ? 6 : 3; // where do the extra bytes start
    ...
    var taskFn = function (start, end) {
        ...
        for (let i = start; i < end; i++) {
            ...
            let p = decomposePoint(b, offr, schema);

            // position
            farray[offw] = b[offr + 0];
            farray[offw + 1] = b[offr + 1];
            farray[offw + 2] = b[offr + 2];

            // all color channels
            for (let ci = 0 ; ci < totalColorChannels ; ci ++) {
                let is = colorizers['colorSource' + ci];
                let col = is == null ? [0, 0, 0] : is.color(p);

                farray[offw + ci * 3 + 3] = col[0];
                farray[offw + ci * 3 + 4] = col[1];
                farray[offw + ci * 3 + 5] = col[2];
            }

            // anything that comes after the position/color is just copied over
            for (let j = 0; j < extra; j++) {
                farray [offw + 3 * totalColorChannels + j] = b[offr + pointExtraOffset + j];
            }
...
var colorizeBuffer = function(schema, b, total, size, bbox, colorizers, params, stats, cb) {
    ...
    async.map(colorizers, (c, cb) => c.prep({
...
var load = function(params, cb) {
    ...
    util.getBinary(u, qs, {creds: params.creds}, function(err, buffer) {
        ...
        var finishWith = function(buffer, stats) {
            ...
            let neededColorChannels = _.size(imagery);
            _.times(neededColorChannels, i => {
                attrib.push(["color" + i, offset, 3]);
                offset += 3;
            });
            ...
            let finishBuffer = function(buffer) {
                ...
                let newPointSize = incomingPointSize + neededColorChannels * 12 - (hasColor ? 12 : 0);

                console.log("-- -- finishing:", incomingPointSize, newPointSize, hasColor, attrib);
                console.log(buffer);
                // figure out all of the colorizers which need to propagate their colors
                //
                var propagationData = [];
                for (let i = 0 ; i < neededColorChannels ; i ++) {
                    let cc = colorizers['colorSource' + i];
                    if (cc && cc.needPropagation()) {
                        propagationData = cc.propagationParamsFromLastPrep();
                        propagationData.pointOffset = 3 + i * 3;
                    }
                }

                console.log("-- -- prop data:", propagationData);
