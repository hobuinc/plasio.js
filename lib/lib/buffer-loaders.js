...
var GreyhoundPipelineLoader = function(server, resource, schema, params) {
    ...
    this.preScale = params.preScale ?
        [params.preScale[0], params.preScale[2], params.preScale[1]] : null;
    this.preOffset = params.preOffset ?
        [params.preOffset[0], params.preOffset[2], params.preOffset[1]] : null;
...
GreyhoundPipelineLoader.prototype.loadHierarchyInfo = function(pointBBox, pointCloudRangeX, depthBegin, depthEnd, cb) {
    ...
    let urlify = (v) => encodeURIComponent(JSON.stringify(v));

    let qs =
        "bounds=" + urlify(bounds) + "&" +
        "depthBegin=" + depthBegin + "&" +
        "depthEnd=" + depthEnd;

    if (this.preScale) {
        var ps = [this.preScale[0], this.preScale[2], this.preScale[1]];
        var po = [this.preOffset[0], this.preOffset[2], this.preOffset[1]];

        ps = ps.map((v) => 1.0 / v);
        po = po.map((v) => -v);

        console.log('HPRE', ps, po);
        qs += "&scale=" + urlify(ps) + "&offset=" + urlify(po);
    }
...
var load = function(params, cb) {
    ...
    //
    var bbox = fixEasting({pointCloudMinX: fullPointCloudRangeX[0],
    ...
    if (params.normalize) {
        // when the user requests normalized coordinates, we need to send down
        // an offset field which gives us points offseted by that value
        let center = vec3.lerp(
                [0, 0, 0],
                params.pointCloudMins,
                params.pointCloudMaxs,
                0.5);

        // center is in our point cloud frame where Y goes up, we need it to be
        // where Z goes up, this shit never ends.
        qs.offset = [center[0], center[2], center[1]];
        qs.bounds = qs.bounds.map((v, i) => v - qs.offset[i % 3]);

        if (params.preScale) {
            qs.scale = [
                1.0 / params.preScale[0],
                1.0 / params.preScale[2],
                1.0 / params.preScale[1]];

            qs.offset = [
                qs.offset[0] - params.preOffset[0],
                qs.offset[1] - params.preOffset[2],
                qs.offset[2] - params.preOffset[1]
            ];
        }
    }
    ...
    util.getBinary(u, qs, {creds: params.creds}, function(err, buffer) {
        ...
        var finishWith = function(buffer, stats) {
            ...
            let finishBuffer = function(buffer) {
                ...
                var channelColorRamps = [];
                var channelRampContribution = [0, 0, 0, 0];
