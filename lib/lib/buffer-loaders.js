...
class BufferLoader {
    constructor(loaderCount) {
        ...
        this.loaders = [];
        const lazPerfLocation = window.LAZPERF_LOCATION || "lib/dist/laz-perf.js";

        /*
        Promise.all(
            this._loadScriptAsBlob(lazPerfLocation),
            this._loadScriptAsBlob(script)
        ).then((blobs) => {
            this.scriptBlob = new Blob(blobs);
        ...
        return new Promise((resolve, reject) => {
            ...
            xhr.onload = function(e) {
                if (this.status == 200) {
                    ...
                    reject(new Error("Failed to download script:" + script));
                }
            };
    ...
    _newWorker(index) {
        /*
        if (!this.scriptBlob)
            throw new Error('Cannot create a downloader worker without an assembled script');

        const U = window.URL || window.webkitURL;
        const w = new Worker(U.createObjectURL(this.scriptBlob));
        */
    ...
    _processResponse(worker, data) {
        ...
        console.log(data);
        console.log(worker.processQueue);
        ...
        if (success) {
            ...
            reject({
                error: new Error('Webworker reported error while trying to process task'),
                task: task
            });
        }
    }
...
DecompressManager.prototype.push = function(schema, buffer, pointsCount, worldBoundsX,
    ...
    return new Promise((resolve, reject) => {
        ...
        var worker = this.free.shift();  // SHIFTs are considered O(n), mmkay? but for short arrays like this its no problem
...
var loader = new BufferLoader(10);
...
export class GreyhoundPipelineLoader extends BaseLoader {
    ...
    static async _internalLoad(params) {
        ...
        console.log("pipeline load:", depthBegin, "->", depthEnd, renderSpaceBounds, treeSpaceBounds);
        ...
        const baseUrl  = util.joinPath(server, "resource", resource, "read");
        ...
        */
        const decompressedData = await loader.push({
            url: baseUrl + "?" + util.makeQueryString(qs),
        ...
        const decompressedBuffer = decompressedData.buffer;
        const pointCount = decompressedData.numPoints;
        const decompressedStats = decompressedData.stats;
