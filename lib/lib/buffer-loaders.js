...
var load = function(params, cb) {
    ...
    if (params.normalize) {
        // when the user requests normalized coordinates, we need to send down an offset
        // field which gives us points offseted by that value
        //
        let center = vec3.lerp([0, 0, 0], params.pointCloudMins, params.pointCloudMaxs, 0.5);

        // center is in our point cloud frame where Y goes up, we need it to be where Z goes up,
