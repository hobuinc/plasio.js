...
export class GreyhoundPipelineLoader extends BaseLoader {
    ...
    constructor(resourceConfig, schema, params) {
        ...
        this.eptHierarchyStep = util.checkParam(params, 'eptHierarchyStep', null);
        this.key = key;
    ...
    /**
     * If the tree is a EPT tree, then this function returns the hierarchy info chunking, if the
     * tree is not EPT, this function will throw an error.
     *
     * @return {Number} A number indicating the hierarchy step, or null if not set
     */
    getEPTHierarchyStep() {
        if (!this.isEPT())
            throw new Error('Only EPT trees support this operation');

        return this.eptHierarchyStep;
    }
    ...
    async loadEPTHierarchyInfo(nodeName) {
        ...
        const url = (nodeName == null) ? util.joinPath(this.eptRootUrl, 'entwine-hierarchy.json')
            : util.joinPath(this.eptRootUrl, 'h', `${nodeName}.json`);
