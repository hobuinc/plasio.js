...
let recolorNode = function(toColor, node, imageData) {
	...
	console.log("total buffers to update:", targetBufs.length);
	targetBufs.forEach(b => {
		let targetBuf = b.data;
		let totalPoints = b.totalPoints;
		let targetps = b.pointSize / 4;
		console.log("total points to check:", totalPoints, " pointsize:", targetps);
		...
		console.log("was buffer touched?", wasBufferTouched);
...
class PointBufferCache {
	...
	print() {
		let p = function(nodes) {
			nodes.forEach(n => {
				console.log(snode(n), n.children.length);
				p(n.children);
			});
		};
	...
	insert(parent, roots, node) {
		...
		if (matching) {
			console.log("++++++++++ FOUND MATCHING QUAD");
	...
	burnImagery(node) {
		// the root itself is correctly burnt up already, we need to walk our way up
		//
		console.log(node);
...
GreyhoundPipelineLoader.prototype.queryFor = function(treeBBox, depthBegin, depthEnd, bulk) {
    return {
		...
		imagery: this.imageLoader.queryFor(treeBBox)
    };
};
...
var load = function(params, cb) {
	...
	if (_.isArray(params.bulk)) {
		qs.bulk = params.bulk.map(n => fixedBBox(params, _.clone(n)));
	}
		...
		async.map(buffers, function(item, done) {
			if (compressed) {
				decompress.push(schema, item.response, item.numPoints,
					[worldMinX, worldMaxX], function(buf) {
						done(null, {
