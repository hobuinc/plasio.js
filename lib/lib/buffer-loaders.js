...
class LocalColorLoader {
	...
	additionalSchemaItems() {
		if (this.subtype === "origin")
			return [["Origin"]];
		return [];
	}
}

let colorSourceFromImagery = function(imagerySource, bbox) {
	let [type, subtype] = splitBy(imagerySource, ".");
	if (type === "mapbox") {
		return new MapboxLoader(bbox, subtype);
	}
	else if (type === "local") {
		return new LocalColorLoader(bbox, subtype);
	}

	return null;
};

var GreyhoundPipelineLoader = function(server, resource, worldBounds, schema, imagerySource) {
    // each loader needs a key
    this.server = server;
    this.resource = resource;
	this.worldBounds = worldBounds;
	this.imageLoader = colorSourceFromImagery(imagerySource, worldBounds);
...
let buildSchema = function(schema, imageLoader) {
	...
	// convert all items to 4 byte floats
	sout = _.map(sout, (s) => {
		if (s.type === "floating" && s.size === 8)
...
GreyhoundPipelineLoader.prototype.queryFor = function(treeBBox, depthBegin, depthEnd) {
	let imageryParams = this.imageLoader.queryFor(treeBBox);
...
GreyhoundPipelineLoader.prototype.setColorSourceImagery = function(source) {
	this.imageLoader = colorSourceFromImagery(source, this.worldBounds);
};
...
var colorizeBufferWithTasks = function(b, total, size, bbox, colorFn, params, cb) {
	if (total === 0)
		return cb(b);

	let actualSize = size / 4; // the point as it came from the source
	let pointSize = actualSize + 3;  // with color component
...
let makeColorizerFromParams = function(imagery, schema, bbox, stats) {
	...
	if (isMapbox(imagery.source)) {
		...
		else if (localImagerySubtype(imagery.source) === "origin") {
			let originOffset = _.findIndex(schema, s => (s.name === "Origin"));
			if (originOffset === -1)
				throw new Error("local.origin color source was requested, but the pipeline schema doesn't seem to have Origin available");

			let colorFn = function(point) {
				let origin = point[originOffset];
				let offset = Math.floor(origin * 1000) % 360;
				return util.hslToRgb(offset/360, 0.5, 0.5);
			};
...
var colorizeBuffer = function(schema, b, total, size, bbox, imagery, params, stats, cb) {
	let colorizer = makeColorizerFromParams(imagery, schema, bbox, stats);
	colorizer(function(colorFn, done) {
		colorizeBufferWithTasks(b, total, size, bbox, colorFn, params, done);
	}, cb);
};
...
var load = function(params, cb) {
	...
	// only add color if the ADD_COLOR switch is on and no color is coming down from the source
	//
	var addColor = ADD_COLOR && !hasColor;
	...
	util.getBinary(u, qs, function(err, buffer) {
		...
		var finishWith = function(buffer, stats) {
			...
			console.log("-- -- TP:", totalPoints, pointSize, hasColor, addColor, hasIntensity);

			let finishBuffer = function(buffer, outparams) {
				let image = outparams.image;
				let needPropagation = outparams.propagate;

				let newPointSize = addColor ? (pointSize + 12) : pointSize;
				buffercache.push(buffer, totalPoints, newPointSize, bbox,
					image,
					needPropagation,
					() => {
						cb(null, {
			...
			if (addColor) {
				...
				colorizeBuffer(schema, buffer, totalPoints, pointSize, bbox, imagery, params, stats, finishBuffer);
			}
			else {
				finishBuffer(buffer);
			}
