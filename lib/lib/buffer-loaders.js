...
class BufferLoader {
    ...
    _processResponse(worker, data) {
        ...
        console.log('buffer-loader: worker ' +  worker.id + ' queue now has: ' + Object.keys(worker.processQueue).length + ' items.');
...
class LocalColorLoader {
     ...
     * @return {*[]} A 2-tuple of 3-tuple arrays representing a pair of RGB triplets.
     */
    channelColorRamp() {
...
export class GreyhoundPipelineLoader extends BaseLoader {
    ...
    static async _internalLoad(params, loadParams) {
        ...
        return new Promise((resolve, reject) => {
            // the buffer cache push is an async operation, but the current buffer returned to us via
            // a callback as soon as its done processing, the push operation then goes on to
            // color other affected nodes.
            buffercache.push(downloadedBufferParams, brushObjects, (buf) => {
