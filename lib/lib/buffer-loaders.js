...
DecompressManager.prototype.push = function(schema, buffer, pointsCount, cb) {
	...
	var dispatch = function(worker, query, done) {
		console.log("WORKER TASK dispatch: ", worker.id);
		worker.postMessage(query);

		worker.onerror = function(e) {
			console.log("WORKER crash: ", worker.id, "will auto-reup");

			// make a new worker and push it back in
			var w = new Worker(o.script);
			w.id = worker.id;
			o.free.push(w);
		};
		...
		worker.onmessage = function(e) {
			...
			console.log("WORKER TASK complete: ", worker.id);
			done(e.data.result);
		};
	};
...
var decompress = new DecompressManager("/workers/decompress.js", 10);
