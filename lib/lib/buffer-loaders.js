...
GreyhoundPipelineLoader.loadInternal = function(obj, cb) {
	...
	//
	var parentLoad = isLoadInProgress(obj);

	if (parentLoad) {
		parentLoad.parked.push(obj);
		printPending();

		return;
	}

	loadsInProgress.push(obj);

	printPending();

	load(obj.params, function(err, res) {
		// this download is done, remove it from in progress downloads
		_.remove(loadsInProgress, n => n == obj);
...
class LoadQueue {
	...
	push(obj) {
		...
		let index = _.findIndex(this.pending, n => n.weight < weight);
		if (index === -1)
	...
	pushReque(tasks) {
		this.pending = this.pending.concat(tasks);
	}
	...
	_startDownloader() {
		...
		var step = function() {
			...
			while (o.pending.length > 0) {
				...
				let {weight, params, cb, bbox} = task;
				...
				if (_isPossiblyAZeroReturn(bbox)) {
					...
					GreyhoundPipelineLoader.loadInternal(task, function(err, res, requeue) {
						cb(null, res);
...
GreyhoundPipelineLoader.load = function(params, cb) {
	...
	let obj = {params: params, cb: cb, weight: weight, parked: [], bbox: bbox};
	_loadQueue.push(obj);
	*/
