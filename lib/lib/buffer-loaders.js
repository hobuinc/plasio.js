...
var GreyhoundPipelineLoader = function(server, resource, schema, params) {
    ...
    // if scale was provided as a scaler, turn it into a vector
    if (this.scale && !_.isArray(this.scale)) {
...
GreyhoundPipelineLoader.prototype.queryFor = function(params) {
    ...
    let scale = this.scale || [1, 1, 1];

    // scale the pointBbox
    var pointMins = pointBBox.mins.slice(0);
    var pointMaxs = pointBBox.maxs.slice(0);

    pointMins[0] = pointMins[0] / scale[0];
    pointMins[1] = pointMins[1] / scale[1];
    pointMins[2] = pointMins[2] / scale[2];

    pointMaxs[0] = pointMaxs[0] / scale[0];
    pointMaxs[1] = pointMaxs[1] / scale[1];
    pointMaxs[2] = pointMaxs[2] / scale[2];
    ...
    return {
        ...
        schema: schema,
        pointCloudMins: fullPointBBox.mins,
        pointCloudMaxs: fullPointBBox.maxs,
...
GreyhoundPipelineLoader.prototype.loadHierarchyInfo = function(pointBBox, pointCloudRangeX, depthBegin, depthEnd, cb) {
    ...
    // if we have a scale, apply it, note that we apply the scale first, since our
    // scale is in Y-up coordinate space, which is the case for bounds as well,
    // the coordinate space is swapped next.
    if (this.scale) {
        for (var i = 0 ; i < bounds.length ; i ++) {
            bounds[i] = bounds[i] / this.scale[i % 3];
        }
    }
...
var colorizeBuffer = function(schema, b, total, size, bbox, colorizers, params, stats) {
    // prep all colorizers which may include fetching imagery
    return new Promise((resolve, reject) => {
        async.map(colorizers, (c, cb) => {
            if (c != null) {
                return c.prep({currentParams: params, stats: stats, bbox: bbox}, resolve);
            }
            resolve();
        }, (err) => {
            // TODO: report error and setup a colorizing chain
            if(err) return console.log("PREP ON COLORIZER FAILED:", err)

            // all good, initiate coloring
            colorizeBufferWithTasks(b, schema, total, size, bbox, colorizers, params, resolve);
        });
    });
};
...
var load = co(function*(params) {
    ...
    let fullPointCloudRangeX = [params.pointCloudMins[0], params.pointCloudMaxs[0]];

    // make sure the bound box is fixed for easting correction
    //
    var bbox = fixEasting({
        pointCloudMinX: fullPointCloudRangeX[0],
        pointCloudMaxX: fullPointCloudRangeX[1]
    ...
    const buffer = yield util.getBinary(u, qs, {creds: params.creds});

    if (buffer.numPoints === 0)
        return { totalPoints: 0 };

    // decompress data, buffer.response is inaccessible beyond this point
    const decompressedData = yield decompress.push(schema, buffer.response, buffer.numPoints, fullPointCloudRangeX,
        params.normalize, params.scale, params.offset);

    const decompressedBuffer = decompressedData.buffer;
    const decompressedStates = decompressedData.stats;
    ...
    if (neededColorChannels > 0) {
        ...
        for(let i = 0 ; i < neededColorChannels ; i ++) {
            attribs.push(["color" + i, offset, 1]);
            offset = offset + 1;
        }

        // get all colorizers we need
        let colorizers = recoverImagerySourceFromParams(imagery);
        let correctedBounds = params.worldMins.concat(params.worldMaxs);

        // do inplace colorization
        yield colorizeBuffer(schema, decompressedBuffer, totalIncomingPoints, incomingPointSize, correctedBounds,
            colorizers, params, stats);
        ...
        yield buffercache.push(buffer, totalIncomingPoints, newPointSize, correctedBounds, propagationData)
        return {
            pointStride: newPointSize,
            totalPoints: totalIncomingPoints,
            attributes: attrib,
            uniforms: [["availableColors", imageryToAvailableColors(imagery)]]
