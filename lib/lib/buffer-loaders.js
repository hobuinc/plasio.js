...
export class GreyhoundPipelineLoader extends BaseLoader {
     ...
     * @param params.brushes {BaseBrush[]} An array of imagery source specifications.
    ...
    static _buildSchema(schema, brushes) {
        ...
        return neededFields.forEach(field => {
     ...
     * @param source {string} The color source specification e.g. local://color.
     */
    ...
    static async _internalLoad(params, loadParams) {
        ...
        const downloadedBufferParams = {
            ...
            pointCloudBufferStats: decompressedStats,
            treePath: treePath,
            schema: schema,
            geoTransform: geoTransform
        };

        await buffercache.push(downloadedBufferParams, brushObjects);
