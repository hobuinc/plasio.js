...
export class PointCloudViewer extends EventEmitter {
    ...
    constructor(targetElement, params) {
        ...
        this.resource = Array.isArray(this.resource) ? this.resource : [this.resource];
    ...
    _baseUrl(resource, ...parts) {
        return util.joinPath.apply(null,
            [util.pickOne(this.server), "resource", resource].concat(parts));
    }
    ...
    async _readResourceConfig(resource) {
        const response = await fetch(this._baseUrl(resource, 'info'),
    ...
    async start() {
        ...
        const allBounds = util.joinBounds(allConfigs.map(c => c.bounds));
        console.log('All configs bounds:', allBounds);

        const allOffset = util.boundsCenter(allBounds);
        const allBoundsGeoTransform = new GeoTransform(allBounds, [0.01, 0.01, 0.01], allOffset);
        ...
        allConfigs.forEach(config => {
            ...
            const [scale, offset] = normalizeScaleAndOffset(state.geoBounds, config.scale, config.offset);

            // get things out
            state.schema = config.schema;
            state.scale = scale;
            state.offset = offset;
            state.numPoints = config.numPoints;
            ...
            const stopSplitDepth = state.numPoints ? Math.ceil((Math.log(state.numPoints) / Math.log(4)) * 1.1) : 16;
            ...
            // report what we got
            console.log("resource parameters (" + config.resource + "): ");
            ...
            //
            const pipelineLoader = new GreyhoundPipelineLoader(this.server,
                state.config.resource, state.schema, {
    ...
    setFilter(filter) {
        ...
        this.policy.hookedReload(() => {
            this.configState.forEach(s => s.pipelineLoader.setFilter(this.filter));
        });
    }
