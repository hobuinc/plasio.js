...
function normalizeScaleAndOffset(scale, offset) {
    ...
    if (scale == undefined || scale == null) {
        ...
        fixedScale = util.geoToRender(scale);
    }
    ...
    if (offset == undefined || offset == null) {
        ...
        fixedOffset = util.geoToRender(offset);
    }
...
export class PointCloudViewer extends EventEmitter {
    ...
    start() {
        return fetch(this._baseUrl('info')) .then((response) => {
            ...
            this.originalBBox = util.geoToRender(config.bounds);
            ...
            this.bbox = [this.originalBBox[0] * scale[0] + offset[0],
                         this.originalBBox[1] * scale[1] + offset[1],
                         this.originalBBox[2] * scale[2] + offset[2],
                         this.originalBBox[3] * scale[0] + offset[0],
                         this.originalBBox[4] * scale[1] + offset[1],
                         this.originalBBox[5] * scale[2] + offset[2]];
            ...
            console.log("... original bbox:", this.originalBBox);
            console.log("... adjusted bbox:", this.bbox);
            ...
            // determine view ranges
            const dx = this.bbox[3] - this.bbox[1],
                  dy = this.bbox[4] - this.bbox[2],
                  dz = this.bbox[5] - this.bbox[3];

            // setup intial camera hinting if no initial camera params are specified
            if (!_.isEmpty(this.initialCameraParams)) {
                modeManager.propagateDataRangeHint(dx, dy, dz);
            }
