...
export class PointCloudViewer extends EventEmitter {
     ...
     * @param [params.disableSplitLimiting] {Boolean} When specified, no split limiting is applied.
    ...
    async _readResourceConfig(r) {
        const {server, resource} = r;
        const response = await fetch(this._baseUrl(server, resource, 'info'),
        ...
        config.schema = config.schema.concat((config.addons || []).map(c => Object.assign({}, c, {addon: true})));
    ...
    async start() {
        ...
        allConfigs.forEach(config => {
            ...
            //
            const pipelineLoader = new GreyhoundPipelineLoader(state.config.server,
