...
export class PointCloudViewer extends EventEmitter {
    ...
    start() {
        return fetch(this._baseUrl('info')) .then((response) => {
            ...
            this.geoTransform = {
                fullGeoBounds: this.geoBounds, scale: this.scale, offset: this.offset
            };

            this.worldBounds = util.transformCoordinateSystem(config.bounds, 'tree', 'render', this.geoTransform);
