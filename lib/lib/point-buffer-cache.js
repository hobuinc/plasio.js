...
export class PointBufferCache {
    ...
    async push(downloadedBufferParams, brushes, bufferReadyCB) {
        ...
        await this._runRecolorTasks(impactedNodes, bufferParams.geoTransform, bufferParams.pointCloudBufferStats);
    }
            ...
            return Promise.delay(0).then(() => {
                ...
                for (let i = start; i < end; i++) {
                    ...
                    for (let bi = 0, bl = cleanedUpBrushes.length; bi < bl; bi++) {
                        ...
                        //if (i < 5) console.log(color.toString(), brush.min, brush.max, brush.scalef);
    ...
    async _recolorNode(node, brush, geoTransform, pointCloudBufferStats) {
        ...
        await brush.prepare(bufferParams, parentNode, childrenNodes);
    ...
    async _runRecolorTasks(impactedNodes, geoTransform, pointCloudBufferStats) {
        // go through all nodes (they are already in the correct traversal order).
        //
        const recolorTasks = [];
        ...
        for (let i = 0, il = impactedNodes.length ; i < il ; i ++) {
            ...
            recolorTasks.push(this._recolorNode(node, brush, geoTransform, pointCloudBufferStats));

            // whenever a node is re-colored we mark it as needing updated, will cause renderer to re-up this
            // buffer to the GPU.
            node.data.update = true;
        }

        await Promise.all(recolorTasks);
    }
}
