...
var ImageLoader = function() {
    this.key = "kitten-images";
    this.provides = "image-overlay";
};

ImageLoader.prototype.load = function(params, cb) {
    var url = "http://localhost:4000/cat.jpeg";
    var img = new Image();
    img.crossOrigin = '';
    img.onload = function() {
        cb(null, img);
...
TransformLoader.prototype.load = function(params, cb) {
...
Cell.prototype._makeId = function(specs) {
    var n = this.worldBBox.mins;
    var x = this.worldBBox.maxs;

    var range = n.map(function(v, i) {
        return x[i] - v;
    });

    var id = {
        "greyhoundstatic" : hashSpecs(specs),
        "kitten-images": {},
        "transform": {
            position: this.worldBBox.center(),
            offset: this.bbox.center(),
            mins: n,
            maxs: x
        }
    };
